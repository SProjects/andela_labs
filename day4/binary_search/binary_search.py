class BinarySearch:
    ...
    def search(self, item):
        try:
            search_item_index = self.created_list.index(item)
        except ValueError:
            search_item_index = -1

        target_list = self._get_target_portion(search_item_index)
        return self._binary_search(item, search_item_index, target_list, 0)
    ...
    def _get_target_portion(self, index):
        ...
        item_percentile = int((float(index)/float(len(self.created_list))) * percentiles) * len(self.created_list)
    ...
    def _binary_search(self, search_item, search_item_index, items, loops):
        ...
        return self._binary_search(search_item, search_item_index, valid_half, loops + 1)
